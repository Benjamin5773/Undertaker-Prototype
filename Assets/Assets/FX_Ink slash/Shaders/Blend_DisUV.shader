    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _Mask_Range)
                UNITY_DEFINE_INSTANCED_PROP( float, _U)
                UNITY_DEFINE_INSTANCED_PROP( float, _V)
                UNITY_DEFINE_INSTANCED_PROP( float, _U_Mask)
                UNITY_DEFINE_INSTANCED_PROP( float, _V_Mask)
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float _U_Mask_var = UNITY_ACCESS_INSTANCED_PROP( Props, _U_Mask );
                float _V_Mask_var = UNITY_ACCESS_INSTANCED_PROP( Props, _V_Mask );
                float2 node_5655 = (i.uv0+(node_9575.g*float2(_U_Mask_var,_V_Mask_var)));
                float4 _Mask_var = tex2D(_Mask,TRANSFORM_TEX(node_5655, _Mask));
                float _Mask_Range_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Mask_Range );
                clip((_Mask_var.r/_Mask_Range_var) - 0.5);
////// Lighting:
////// Emissive:
                float4 node_8161 = _Time;
                float _U_var = UNITY_ACCESS_INSTANCED_PROP( Props, _U );
                float _V_var = UNITY_ACCESS_INSTANCED_PROP( Props, _V );
                float2 node_955 = (i.uv0+(node_8161.g*float2(_U_var,_V_var)));
                float4 _TEX_var = tex2D(_TEX,TRANSFORM_TEX(node_955, _TEX));
                float4 _Color_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Color );
                float3 emissive = (_TEX_var.rgb*_Color_var.rgb*i.vertexColor.rgb);
                float3 finalColor = emissive;
                fixed4 finalRGBA = fixed4(finalColor,(_TEX_var.a*_Color_var.a*i.vertexColor.a));
